...
import { Monitor, Maximize, Minimize, Radio, StopCircle } from 'lucide-react';

interface Props { token: string }

export function LiveScreenTab({ token }: Props) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-surface-900 dark:text-white">بث الشاشة المباشر</h2>
        ...
        <div className="flex gap-2">
          ...
          {isActive ? (
            ...
            <Button size="sm" icon={<Radio size={16} />} onClick={startStream} loading={isLoading}>بدء البث</Button>
          )}
        </div>
      </div>
